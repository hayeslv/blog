  ...
  <div class="wrap">
    <PanelBac title="滚动导航" width="100%" :height="210">
      <div class="bottom-nav">
        <div class="nav-wrap" :style="{ transform: `translateX(-${xPoint}px)`, 'transition-duration': isOpenTransform ? '0.3s' : '0s' }">
          <div v-for="(nav, index) in navs" :key="index" class="nav" :class="{ active: curIndex === index }" @click="routeTo(nav.name, index)">
            {{ nav.label }}
            <div class="triangle" :class="{ isshow: $route.name === nav.name && curIndex === index }" />
